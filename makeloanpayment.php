...
if(isset($_POST['submit']))
{
	...
	if(mysqli_affected_rows($con) == 1)
	{
		if(($lastinsid == $_GET["lins_id"]) && ($_POST['balamts'] == $_POST['paymentamt']))
		{
				...
				$msg= str_replace(" ","%20","Your Loan payment to $txtloanacno has been made on "  . $dts  . " for the amount of Rs. ". $paymentamt. " and Your Loan account has been closed. Thank You for Payment.. -PERKSFinance");
				$ch = curl_init();
		...
		else
		{
				...
				$paymenttype  = $_POST['paymenttype'];
				$dts = date("d M Y",strtotime($dt));
				$msg= str_replace(" ","%20","Your payment to $txtloanacno (Installment No. $txtinstno ) has been made on "  . $dts  . " for the amount of Rs. ". $paymentamt. ". Thank You for Payment.. -PERKSFinance");
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Length: 0'));
				curl_setopt($ch,CURLOPT_URL,  "https://www.smsgatewayhub.com/api/mt/SendSMS?APIKey=04024e1f-2d50-4874-b1b5-b1b1d901e928&senderid=BIXTEL&channel=2&DCS=0&flashsms=0&number=$mobno&text=$msg&route=21");
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
<form method="post" action="" onsubmit="return validateform()" >
 ...
 <h3 class="heading3"> Loan Payment Panel</h3>
		...
		<tbody>
			...
			while($rs = mysqli_fetch_array($qsql))
			{
				...
				echo "<tr>
						...
						<td style='text-align: right'>" . moneyFormatPhil($rs['lins_amt']) ."</td>
						<td style='text-align: right'>" . moneyFormatPhil($rs['lins_iamt']) ."</td>
						<td style='text-align: right'>". moneyFormatPhil($rsloandelaypmttransaction['interstamt']) ."</td>
						<td style='text-align: right'>" . moneyFormatPhil($rsloanpenaltytransaction['penaltyamt']) . "</td>
						<td style='text-align: right'>" . moneyFormatPhil($totamt) . "</td>
						<td style='text-align: right'>" . moneyFormatPhil($rsloanpmt['loanpaid']) . "</td>";
						$balamts = $totamt - $rsloanpmt['loanpaid'];
				echo "<td style='text-align: right'>" . moneyFormatPhil($balamts) . "</td></tr>";
			}
			?>
		</tbody>
	</table>
